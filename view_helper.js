...
function make_user_elem(id_prefix, uname, user_attributes=null, use_checkbox=false) {
    if (use_checkbox) {
        user_elem = $(`<div class="ui-widget-content perm_user_checkbox_row" id="${id_prefix}_${uname}" name="${uname}" style="padding: 5px; display: flex; align-items: center;">
            <input type="checkbox" id="${id_prefix}_${uname}_checkbox" name="${id_prefix}_checkbox" style="margin-right: 8px;"/>
            <span id="${id_prefix}_${uname}_icon" class="oi ${is_user(all_users[uname])?'oi-person':'oi-people'}" style="margin-right: 5px;"/> 
...
function define_grouped_permission_checkboxes(id_prefix, which_groups = null) {
    ...
    for(let g of which_groups){
        let row = $(`<tr id="${id_prefix}_row_${g}">
            ...
            <td id="${id_prefix}_${g}_permission_cell" style="text-align: center;">
                <div class="perm_toggle_container" style="display: inline-flex; align-items: center;">
                    ...
                    <label class="perm_toggle_switch" id="${id_prefix}_${g}_toggle_switch" group="${g}">
                        <input type="checkbox" id="${id_prefix}_${g}_permission_toggle" class="perm_toggle_checkbox" group="${g}">
    ...
    let update_group_checkboxes = function(){
        ...
        if( username && username.length > 0 && (username in all_users) &&
            ...
            for(let g of which_groups) {
                let toggle_checkbox = group_table.find(`#${id_prefix}_${g}_permission_toggle`)
                let has_allow = grouped_perms.allow[g] && grouped_perms.allow[g].set
                let has_deny = grouped_perms.deny[g] && grouped_perms.deny[g].set
                
                // Get file object to check inheritance
                let file_obj = path_to_file[filepath]
                
                if (has_allow) {
                    // Allow = checked (right side)
                    toggle_checkbox.prop('checked', true)
                    if (grouped_perms.allow[g].inherited) {
    ...
    group_table.find('.perm_toggle_checkbox').change(function(){
        ...
        toggle_permission_group(filepath, username, group, opposite_type, false)
        toggle_permission_group(filepath, username, group, selected_type, true)
        
        update_group_checkboxes()// reload toggles (this will also check and highlight remove button)
    })
